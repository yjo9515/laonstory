...
//
        ...
        for (int k = 0; k < 2; k++) {
            ...
            Log.e(TAG, "connect:超高频连接串口  - -  " +uhfConfig.getUhfUart() );
            boolean result = UHFSerialPort.getInstance().open(uhfConfig.getUhfUart(), uhfProtocolAnalysisSLR, baudrate);
            LoggerUtils.d(TAG, "시리얼포드 열림 여부=" + result + "  Uart=" + uhfConfig.getUhfUart() + "  baudrate=" + baudrate);
            if (!result) {
                //打开串口失败
...
//                    hcPowerCtrl.uhfCtrl(0);
        ...
        if (verInfo.getResultCode() != UHFReaderResult.ResultCode.CODE_SUCCESS) {
            ...
            LoggerUtils.d(TAG, "Firmware version: " + firmwareVersion + " Hardware version=" + hver);
