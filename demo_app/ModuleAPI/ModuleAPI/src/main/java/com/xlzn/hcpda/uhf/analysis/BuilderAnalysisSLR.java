...
//                2667 CRC
                ...
                LoggerUtils.d(TAG, "user 数据长度 = " +userLen);
...
//
    ...
    public UHFReaderResult<UHFVersionInfo> analysisVersionData(DataFrameInfo data) {
        if (data != null) {
            if (data.status == 0) {
                ...
                LoggerUtils.d(TAG, "펌웨어 버전:" + versionInfo.getFirmwareVersion());
                LoggerUtils.d(TAG, "하드웨어버전:" + versionInfo.getHardwareVersion());
    ...
    public byte[] buildSendData(int cmd, byte[] data) {
        LoggerUtils.d(TAG,"데이터 조합-------"+DataConverter.bytesToHex(data));
        int index = 0;
        if (data != null && data.length > 0) {
            ...
            LoggerUtils.d(TAG, "전송 데이터 빌드 buildSendData=>" + DataConverter.bytesToHex(sendData));
            return sendData;
        } else {
