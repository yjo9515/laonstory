...
import { personalChangePasswordApi, companyChangePasswordApi } from "../../../api/MyInfoApi";
...
export default function ChangePasswordModal({ type }: { type: string }) {
  ...
  const { register, handleSubmit, setValue, watch } = useForm<IChangePassword>();

  // pass 정보
  const [passData, setPassData] = useState<string>("");

  // personal 비밀번호 변경 api
  const changePersonalPassword = useSendData(personalChangePasswordApi);
  // company 비밀번호 변경 api
  ...
  const onValid: SubmitHandler<IChangePassword> = (formData) => {
    const { newPassword, confirmNewPassword, password, phone, encData } = formData;
    ...
    // 정보 수정 api 실행
    const data = { newPassword, encData, password };
    if (type === "personal") {
      changePersonalPassword.mutate(data, {
        onSuccess() {
          // api 성공 시
            ...
            <div>
              <InputSet
                ...
                register={register("newPassword", {
                  required:
                    "변경할 비밀번호를 입력하세요.(영문, 숫자, 특수문자(!,@,#,$,%,?,&,*) 포함 9자리 이상)",
