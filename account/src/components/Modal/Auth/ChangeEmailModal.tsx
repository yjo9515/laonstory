...
import { personalChangeEmailApi, companyChangeEmailApi } from "../../../api/MyInfoApi";
...
import { IPersonalChangeEmailData } from "../../../interface/MyInfo/ApiData";
...
export default function ChangeEmailModal({ type }: { type: string }) {
  ...
  // personal 이메일 변경 api
  const personalChangeEmail = useSendData<string, IPersonalChangeEmailData>(personalChangeEmailApi);
  // company 이메일 변경 api
  const companyChangeEmail = useSendData<string, IPersonalChangeEmailData>(companyChangeEmailApi);
  ...
  const onValid: SubmitHandler<IChangeEmail> = (formData) => {
    const { newEmail, emailCheck, uuid, password } = formData;
    ...
    if (!emailCheck) {
      setAlertModal({
        isModal: true,
        content: "이메일 인증을 완료하세요.",
      });
      return;
    }
    const data = { email: newEmail, uuid, password };
    // email 인증번호 확인 api 실행
    if (type === "personal") {
      personalChangeEmail.mutate(data, {
        onSuccess() {
    ...
    if (type === "company") {
      companyChangeEmail.mutate(data, {
        onSuccess() {
