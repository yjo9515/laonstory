...
export default function LoginFind() {
  ...
  useEffect(() => {
    const listener = (event: StorageEvent) => {
      ...
      if (pass) {
        ...
        setValue("encData", String(data.encData));
  ...
  const onValid: SubmitHandler<ILoginFindInput> = (formData) => {
    ...
    if (findType === "password") {
      const { phoneCheck, newPassword, confirmNewPassword, id, phone, encData } = formData;
      const data = { id, password: newPassword, encData: String(encData) };
      if (!phoneCheck) {
        setAlertModal({
